function password2Changed() {
        ...
        if (pwd.value.length == 0) {
           ...
           $('#submit_signup').attr('disabled', 'disabled')
            ...
            strength.innerHTML = '<span style="color:#cc8e51">Must contain at least one number and one uppercase letter, one lowercase letter and one special character(_,@,-), and at least 8 or more characters</span>';
            $('#submit_signup').attr('disabled', 'disabled')
            
        } else if (strongRegex.test(pwd.value)) {
            strength.innerHTML = '<span style="color:#cc8e51">Strong!</span>';
            $('button[type="submit"]').removeAttr('disabled')
        } else if (mediumRegex.test(pwd.value)) {
            strength.innerHTML = '<span style="color:#e9e9e9">Medium!</span>';
            $('button[type="submit"]').removeAttr('disabled')
        } else {
            strength.innerHTML = '<span style="color:red">Weak! Must contain at least one number and one uppercase letter, one lowercase letter and one special character(_,@,-), and at least 8 or more characters</span>';
            $('#submit_signup').attr('disabled', 'disabled')
                        ...
                        if (minutes < 0) {
                            ...
                            $('#resend_otp').on('click', function () {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        console.log(data)
                                        $("#otp_success").html("An OTP as been resent to your E-mail Id. If you not see the OTP in your 'primary inbox' then checck your 'spam' folder. Please enter the OTP below.")
                                        $('#resend_otp').remove();
